...
import { CommentReplyTarget } from "./CommentMediaPicker";

/**
 * Hold-to-talk voice recorder for comments — an exact fork of
 * components/chat/WeChatVoiceRecorder.tsx (same dome/pill gesture UI,
 * animation, and hold/slide-to-cancel/lock behavior), adapted only where it
 * touches persistence: uploads to the comment-audio bucket and posts via
 * commentsService.addPostComment/addReply instead of inserting into the
 * chat `messages` table. See that file for the geometry/animation math —
 * intentionally left untouched here rather than re-derived.
 */
type CommentVoiceRecorderProps = {
  currentUser: { id: string; name: string; avatar_url?: string | null };
  postId: string;
  postOwnerId: string;
  /** When set, the recorded voice note attaches to a reply on this comment instead of a new top-level comment. */
  replyTarget?: CommentReplyTarget | null;
  onOptimistic: (item: PostComment | CommentReply, isReply: boolean) => void;
  onUploadSuccess: (item: PostComment | CommentReply, optimisticId: string, isReply: boolean) => void;
  onUploadError: (optimisticId: string, isReply: boolean) => void;
...
export default function CommentVoiceRecorder({
  currentUser,
  postId,
  postOwnerId,
  replyTarget,
  ...
  const startRecording = async () => {
    ...
    if (permission.status !== "granted") {
      ...
      setPopup({
        ...
        message: "Please allow microphone access to send voice comments.",
      });
      return;
    }
  ...
  const uploadAudio = async (uri: string, duration: number) => {
    ...
    try {
      ...
      onOptimistic(optimisticItem, isReply);

      const folder = isReply ? `replies/${replyTarget!.commentId}` : `posts/${postId}`;
      const filePath = `${folder}/${optimisticId}_${Date.now()}.m4a`;
      ...
      const {
        data: { publicUrl },
      } = supabase.storage.from("comment-audio").getPublicUrl(filePath);

      const mediaAttachment: CommentMediaAttachment = { url: publicUrl, type: "audio", duration };
      ...
      const result = isReply
        ...
        : await addPostComment(postId, currentUser.id, "", mediaAttachment);

      if (!result) throw new Error("Failed to save voice comment");

      onUploadSuccess(result, optimisticId, isReply);
    } catch {
      onUploadError(optimisticId, isReply);
    } finally {
