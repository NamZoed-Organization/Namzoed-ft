...
export interface FeedAspectReframeResult {
  uri: string;
  width: number;
  height: number;
}

interface FeedAspectReframeOverlayProps {
  visible: boolean;
  imageUri: string;
  imageWidth?: number;
  imageHeight?: number;
  /** Feed frame width ÷ height (e.g. 4/5 portrait, 1 square). */
  aspectWidthOverHeight: number;
  onSave: (result: FeedAspectReframeResult) => void;
  onCancel: () => void;
}

/**
 * Pinch / pan reframing inside a fixed feed aspect (Instagram-style crop step).
 */
export default function FeedAspectReframeOverlay({
  visible,
  imageUri,
  imageWidth,
  imageHeight,
  ...
  onSave,
  onCancel,
}: FeedAspectReframeOverlayProps) {
  const aspect = clampMediaRatio(aspectWidthOverHeight);
  ...
  }, [visible, imageUri, imageWidth, imageHeight, aspect, frameW, frameH]);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      console.error("FeedAspectReframeOverlay save:", e);
      setPopup({
        visible: true,
        type: "error",
        title: "Save Failed",
  ...
  const stageH = stageLayout.h || sh * 0.65;
