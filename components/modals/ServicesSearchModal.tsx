...
type Props = {
  visible: boolean;
  onClose: () => void;
  /** If provided, restrict results to services in this category. */
  categorySlug?: string;
  placeholder?: string;
};
...
function scoreService(s: ProviderServiceWithDetails, q: string): number {
  ...
  // Highest priority: name exact / prefix
  ...
  // Closest match via Levenshtein on words
  const dist = Math.min(
...
export default function ServicesSearchModal({
  visible,
  onClose,
  categorySlug,
    ...
    const load = async () => {
      ...
      try {
        const data = categorySlug
          ? await fetchProviderServicesByCategory(categorySlug)
          : await fetchAllProviderServices(0, 500);
        if (!cancelled) {
          setServices((data || []).filter((s) => s.status !== false));
        }
      } catch (e) {
        console.error("ServicesSearchModal load error:", e);
  ...
  return (
    ...
    >
      <View style={{ flex: 1, backgroundColor: "#ffffff", paddingTop: 44 }}>
        {/* Search header */}
        <View
          style={{
            flexDirection: "row",
